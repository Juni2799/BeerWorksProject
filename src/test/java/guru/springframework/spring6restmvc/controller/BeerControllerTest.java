...
@Import(SpringSecurityConfig.class) //imported for implementing the beans within
class BeerControllerTest {
    ...
    //ObjectMapper is used to create Java POJO to JSON and vice-versa. By auto wiring ObjectMapper in Spring Context,
    //we get an ObjectMapper instance which is autoconfigured by Spring with certain console display formats which we
    ...
    void getBeerById() throws Exception {
        BeerDTO testBeerDTO = beerServiceImpl.getBeers(null, null, false, 1, 25).getContent().get(0);
    ...
    public void createBeerTest() throws Exception {
        BeerDTO beerDTO = beerServiceImpl.getBeers(null, null, false, 1, 25).getContent().get(0);
        beerDTO.setVersion(null);
        beerDTO.setId(null);

        given(beerService.saveNewBeer(any(BeerDTO.class))).willReturn(beerServiceImpl.getBeers(null, null, false, 1, 25)
                .getContent().get(1));
